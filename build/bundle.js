...
function frameIndexToTimeMs(frameIndex, fps) {
  assertPositiveInteger(frameIndex, "frameIndex");
  assertPositiveNumber(fps, "fps");
  return frameIndex * 1e3 / fps;
}
function timeMsToFrameIndex(timeMs, fps) {
  assertPositiveNumber(timeMs, "timeMs");
  assertPositiveNumber(fps, "fps");
  return Math.floor(timeMs / (1e3 / fps));
}
function clamp01(value) {
  return Math.max(0, Math.min(1, value));
}
function resolveBeatState(timeMs, { bpm, offsetMs = 0 }) {
  assertPositiveNumber(bpm, "bpm");
  const beatDurationMs = 6e4 / bpm;
...
function resolveBarState(beatState, timeSignature = [4, 4]) {
  const beatsPerBar = Number(timeSignature == null ? void 0 : timeSignature[0]) || 4;
...
function applyEasing(progress, easingType = "linear") {
...
function useBeatClock({ bpm, offsetMs }) {
...
function goldenSection(width, height) {
  return {
    x: roundTo(width / GOLDEN_RATIO, 2),
    y: roundTo(height / GOLDEN_RATIO, 2)
  };
}
function fibonacciSequence(n, start = 1) {
  const seq = [start];
  let a = start, b = start;
  for (let i = 1; i < n; i++) {
    const next = a + b;
...
function goldenRingRadii(baseRadius, count) {
  const radii = [];
...
const DEFAULT_PIXELBRAIN_CANVAS = Object.freeze({
  width: 160,
  height: 144,
  gridSize: 1
});
const PALETTE_CONTRACT = Object.freeze({
  TIERS: {
  ...
  ADDRESSING: {
    PAGE_SIZE: 8,
    // 8-byte addressing blocks for SSD alignment
    BLOCK_SIZE: 64
  }
});
function shared_clamp01(value) {
...
function parseBytecodeString(bytecode) {
...
const VOWEL_FAMILY_TO_SCHOOL = Object.freeze({
  ...
  // folds to OW
  OO: "VOID",
  // folds to UH (the "book" vowel)
  YUW: "ABJURATION",
  // folds to UW
  YOO: "ABJURATION",
  // folds to UW
  YOW: "ABJURATION",
  // folds to OW (rare y-glide variant)
  EE: "PSYCHIC",
  // folds to IY
  IN: "SONIC",
  // folds to IH
  A: "NECROMANCY",
  // bare "A" shorthand folds to AA
  AI: "PSYCHIC",
  // alternate romanization of AY
  OI: "ALCHEMY",
  // alternate romanization of OY
  OU: "ABJURATION",
  // British "ou" folds to OW
  OUR: "SONIC"
  // "our" reduced form folds to UR
});
...
function computeSchoolWeightsFromHints(tokenHints) {
  const safeHints = Array.isArray(tokenHints) ? tokenHints : [];
  if (safeHints.length === 0) return { VOID: 1 };
  const schoolWeights = {};
  let totalWeight = 0;
  safeHints.forEach((hint) => {
    const schoolId = String((hint == null ? void 0 : hint.schoolId) || "VOID").trim().toUpperCase() || "VOID";
    const weight = 0.4 + (Number(hint == null ? void 0 : hint.anchorWeight) || 0) + (String((hint == null ? void 0 : hint.effect) || "INERT") !== "INERT" ? 0.35 : 0);
    schoolWeights[schoolId] = (schoolWeights[schoolId] || 0) + weight;
    totalWeight += weight;
  });
  if (totalWeight > 0) {
...
function computeDominantSchool(schoolWeights) {
  var _a;
  if (!schoolWeights || Object.keys(schoolWeights).length === 0) return null;
  return ((_a = Object.entries(schoolWeights).sort((left, right) => {
...
function KineticWord({ wordTiming, clock, beatDurationMs }) {
  ...
  if (localT > 1) {
    const r1 = (0,esm.random)(`${wordTiming.word}-${wordTiming.startMs}-r1`);
    const r2 = (0,esm.random)(`${wordTiming.word}-${wordTiming.startMs}-r2`);
    ...
    }, children: wordTiming.word.replace(/./g, "\u2022") });
  }
  ...
  const isGlitching = isEntering && (0,esm.random)(`glitch-${glitchSeed}`) > 0.6;
  const glitchOffsetX = isGlitching ? ((0,esm.random)(`x-${glitchSeed}`) - 0.5) * 15 : 0;
  const glitchOffsetY = isGlitching ? ((0,esm.random)(`y-${glitchSeed}`) - 0.5) * 15 : 0;
  const school = SCHOOLS[wordTiming.school] ?? SCHOOLS.VOID;
  const color = school.color;
  const glyph = SCHOOL_GLYPHS[wordTiming.school] ?? SCHOOL_GLYPHS.VOID;
  const filter = "saturate(0.5)";
  const opacity = localT < 0 ? 0 : 1;
  const msSinceWordStart = Math.max(0, clock.timeMs - wordTiming.startMs);
  ...
  const glyphOpacity = 0.6 * (1 - glyphBloom);
  const glyphScale = 1 + 0.6 * glyphBloom;
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(
    "span",
    {
      ...
      children: [
        ...
        displayedWord,
        /* @__PURE__ */ (0,jsx_runtime.jsx)("span", { style: { visibility: "hidden" }, children: wordTiming.word.substring(charsToShow) })
      ]
    }
  );
}
...
function PixelBrainStage({ activeWords, clock }) {
  const isInstrumental = activeWords.length === 0;
  const beatPulse = Math.pow(1 - clock.beat.phase, 4);
  const barPulse = Math.pow(1 - clock.bar.phase, 6);
  const intensityMultiplier = isInstrumental ? 2 : 1;
  const auroraColor = "#94a3b8";
  const auroraIntensity = clamp01(0.1 + barPulse * 0.5 * intensityMultiplier);
  const auroraAlpha = Math.round(auroraIntensity * 255).toString(16).padStart(2, "0");
  const vignette = clamp01(0.6 + beatPulse * 0.4 * intensityMultiplier);
  const scanlineOpacity = clamp01(0.1 + beatPulse * 0.3 * intensityMultiplier);
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { pointerEvents: "none" }, children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(
      esm.AbsoluteFill,
      {
        style: {
...
function isWordActive(startMs, endMs, nowMs) {
  return startMs < nowMs + 500 && endMs > nowMs - 100;
}
function KineticLyricsVideo({ track }) {
  const pacing = track.pacing ?? DEFAULT_PACING;
  const offsetMs = (pacing.leadInS ?? 0) * 1e3;
  const clock = useBeatClock({ bpm: pacing.bpm, offsetMs });
  const beatDurationMs = 6e4 / pacing.bpm;
  const words = track.wordTimings ?? [];
  const activeWords = words.filter(
    (w) => isWordActive(w.startMs, w.endMs, clock.timeMs)
  );
  const panSeed = clock.bar.index;
  const panX = panSeed * 13 % 40 - 20;
  const panY = panSeed * 7 % 30 - 15;
  let fontWeight = 400;
  if (clock.beat.phase < 0.2) {
    fontWeight = 900;
  } else if (clock.beat.phase < 0.5) {
    fontWeight = 700;
  }
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(
    esm.AbsoluteFill,
    {
      ...
      children: [
        /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Audio, { src: track.audioUrl }),
        /* @__PURE__ */ (0,jsx_runtime.jsx)(PixelBrainStage, { clock, words, activeWords }),
        /* @__PURE__ */ (0,jsx_runtime.jsx)(
          esm.AbsoluteFill,
          {
            style: {
              transform: `translate(${panX}px, ${panY}px) scale(1.05)`,
            ...
            children: /* @__PURE__ */ (0,jsx_runtime.jsx)(
              esm.AbsoluteFill,
              {
                ...
                children: activeWords.map((timing) => /* @__PURE__ */ (0,jsx_runtime.jsx)(
                  KineticWord,
                  {
                    wordTiming: timing,
                    clock,
        ...
        /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: {
          ...
          /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { children: [
            "BAR: ",
            clock.bar.index.toString().padStart(3, "0"),
            " | BEAT: ",
            clock.beat.index.toString().padStart(3, "0")
          ] }),
          /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { fontSize: 16, opacity: 0.5 }, children: [
            "PHASE: ",
            clock.beat.phase.toFixed(3)
          ] }),
          /* @__PURE__ */ (0,jsx_runtime.jsxs)("div", { style: { fontSize: 16, opacity: 0.5 }, children: [
            "ACTIVE: ",
            activeWords.length,
...
function typographyMoviePlugins_clamp01(value) {
  return Math.max(0, Math.min(1, value));
}
...
function signedUnit(seed) {
  return stableHash(seed) % 2e3 / 1e3 - 1;
}
function easeOutCubic(value) {
  const p = typographyMoviePlugins_clamp01(value);
...
function resolveActiveCueIntensity(packet, types) {
  return packet.cues.filter((cue) => types.includes(cue.type)).reduce((total, cue) => {
    var _a;
    return total + (cue.eased ?? cue.progress) * (((_a = cue.params) == null ? void 0 : _a.intensity) ?? 1);
  }, 0);
}
...
function drawMazeCorridor(ctx, width, height, packet, settings) {
  ...
  const cueTurn = (((_a = resolveCue(packet, "CAMERA_TURN_RIGHT")) == null ? void 0 : _a.eased) || 0) - (((_b = resolveCue(packet, "CAMERA_TURN_LEFT")) == null ? void 0 : _b.eased) || 0);
  const turn = typographyMoviePlugins_clamp01(Math.abs(barTurn * 0.38 + cueTurn * 0.62)) * Math.sign(barTurn + cueTurn || 1);
  ...
  const doorPulse = typographyMoviePlugins_clamp01(gate) * (0.55 + 0.45 * Math.sin(packet.music.beatPhase * Math.PI));
  ctx.strokeStyle = `rgba(214, 179, 95, ${0.42 + doorPulse * 0.44})`;
  ctx.lineWidth = Math.max(2, width * 3e-3);
...
function drawGlyph(ctx, glyph, x, y, fontSize, color, alpha, rotation = 0) {
  ...
  ctx.translate(x, y);
  ctx.rotate(rotation);
  ctx.globalAlpha = typographyMoviePlugins_clamp01(alpha);
...
function drawSnowField(ctx, width, height, packet, progress) {
  ...
  for (let i = 0; i < 70; i += 1) {
    ...
    const y = (stableHash(`${seed}:y`) % height + packet.timeMs * (0.012 + i % 5 * 4e-3)) % height;
...
function drawSnowAngelIntro(ctx, width, height, packet, settings) {
  ...
  const progress = typographyMoviePlugins_clamp01(packet.timeMs / introDuration);
  const approachProgress = typographyMoviePlugins_clamp01(packet.timeMs / Math.max(1, approachMs));
  const holdProgress = typographyMoviePlugins_clamp01((packet.timeMs - approachMs) / Math.max(1, introDuration - approachMs));
  const approach = easeOutCubic(approachProgress);
  const stare = typographyMoviePlugins_clamp01(holdProgress / 0.16);
  const melt = typographyMoviePlugins_clamp01((holdProgress - 0.55) / 0.45);
  const cx = width / 2;
  const groundY = height * (0.76 - approach * 0.22 + melt * 0.04);
  const scale = width * (0.052 + approach * 0.24);
  const alpha = typographyMoviePlugins_clamp01(0.24 + approach * 0.9 - melt * 0.86);
  ...
  if (((_a = settings.introImage) == null ? void 0 : _a.complete) || ((_b = settings.introImage) == null ? void 0 : _b.naturalWidth)) {
...
function drawMazeTypography(ctx, width, height, packet, settings) {
  ...
  glyphs.forEach((glyph, index) => {
    ...
    const alpha = 0.14 + z * 0.68 + typographyMoviePlugins_clamp01(wordImpact) * 0.22;
    const tremor = settings.motion === "reduced" ? 0 : Math.sin(packet.music.beatPhase * Math.PI * 2 + index) * width * 6e-3;
    if (side === 0) {
      drawGlyph(
        ctx,
        glyph,
    ...
    if (rhymeEcho > 0.05) {
      drawGlyph(
        ...
        "#ffffff",
        typographyMoviePlugins_clamp01(rhymeEcho * 0.18),
        laneSeed * 0.28
      );
    }
  });
  ...
  if (lyricText) {
    ...
    const progress = typographyMoviePlugins_clamp01(activeProgress / lyricWeight);
    const titleSize = Math.min(width * 0.085, 78) * (1 + typographyMoviePlugins_clamp01(wordImpact) * 0.1);
    ctx.save();
    ctx.font = `900 ${titleSize}px Georgia, serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.shadowColor = "#d6b35f";
    ctx.shadowBlur = titleSize * (0.28 + typographyMoviePlugins_clamp01(gateOpen) * 0.18);
    ctx.fillStyle = "#f7e8b4";
    ctx.globalAlpha = 0.28 + progress * 0.54 + typographyMoviePlugins_clamp01(gateOpen) * 0.18;
    ctx.fillText(lyricText.slice(0, 34), cx, height * (0.49 - typographyMoviePlugins_clamp01(gateOpen) * 0.06));
    ctx.restore();
  }
}
...
const TYPOGRAPHY_MOVIE_PLUGINS = Object.freeze([
  Object.freeze({
...
function getTypographyMoviePlugin(pluginId) {
...
const mazePlugin = getTypographyMoviePlugin("first-person-maze");
function MazeLyricsVideo_isWordActive(startMs, endMs, nowMs) {
  return startMs < nowMs + 500 && endMs > nowMs - 2e3;
}
function buildMazePacket(trackId, frame, bpm, clock, activeWords) {
  const cues = [];
...
function MazeLyricsVideo({ track }) {
  const pacing = track.pacing ?? DEFAULT_PACING;
  const offsetMs = (pacing.leadInS ?? 0) * 1e3;
  const clock = useBeatClock({ bpm: pacing.bpm, offsetMs });
  const frame = (0,esm.useCurrentFrame)();
  ...
  const words = track.wordTimings ?? [];
  const activeWords = words.filter((w) => MazeLyricsVideo_isWordActive(w.startMs, w.endMs, clock.timeMs));
  ...
  return /* @__PURE__ */ (0,jsx_runtime.jsxs)(esm.AbsoluteFill, { style: { background: "#05050b" }, children: [
    /* @__PURE__ */ (0,jsx_runtime.jsx)(esm.Audio, { src: track.audioUrl }),
    /* @__PURE__ */ (0,jsx_runtime.jsx)(
      "canvas",
      {
...
const PETRICHOR = {
  ...
  coverUrl: "https://cdn2.suno.ai/video_gen_4e090dfa-e348-4e23-b00f-20269bf8e2ca_video_upload_4e090dfa-e348-4e23-b00f-20269bf8e2ca_cover_snapshot_0s_1781109985_image.jpeg",
  meta: [
    ["Duration", "4:01"],
    ["Model", "chirp-fenix \xB7 v5.5"],
    ["Persona", "Vaelrix"],
    ["Style", "Cinematic emo rock \xD7 hyperpop"],
    ["Released", "June 10, 2026"],
    ["Source", "suno.com"]
  ],
  provenance: {
    statement: "Crafted with human intention and AI assistance.",
    tools: ["Suno v5.5 \xB7 chirp-fenix", "Persona \u2014 Vaelrix"],
    assistance: "Cinematic emo rock \xD7 hyperpop \xB7 glitch intro \xB7 reverb-washed textures"
  },
  lyrics: [
    "I don't care about scar tissues",
    "I just care that I miss you",
    "I prepared my life knowing my life is no longer yours",
    "but I love you enough just to keep it alive",
    "cause' nobody is a measure to you",
    "and I tried to remember the bruises",
    "Tried to remember the truth",
    "and then I did",
    "And the end result, is there's nobody like you...",
    "So I wait for the call.",
    "Wait for the message. Wait for response.",
    "but I can't even stop ruminating every day seeing your face",
    "but I stopped reaching out, a mistake.",
    "Haven't touch a woman since,",
    "They are not my home, and it doesn't make sense,",
    "like you",
    "It never made sense like you",
    "I can't pretend that they do.",
    "emotions are too strong to tame,",
    "and I can't erase the way I want you,",
    "but it's too late",
    "I'm a few screws too loose,",
    "nuclear suitcase",
    "Stupid, it's too fake to ever let go when my feelings are my real face.",
    "and I'm not scared to admit that I love you",
    "And that nobody else is ever above you",
    "probably die from emotional struggle",
    "I'd rather hold on than admit that we're rubble",
    "and maybe I'm dumb. Maybe the muscle",
    "of my heart real strong when my mind is crushed though",
    "I grew my hair longer just like Repunzel",
    "because you loved it, and I love what you love too.",
    "I still separate the purple skittles.",
    "I still cook eggs with the cheese that you like",
    "And it's hurts to remember the things I went through",
    "to love you enough that I shared my life",
    "I'm drowning in sorrow, The grief of tomorrow",
    "not being near you and having no bottles",
    "I even quit weed just to process the heart break",
    "I screamed to the sky and I witnessed the stars shake",
    "I wanted to love you as much as I could",
    "couldn't let go of the allure of being misunderstood",
    "Kisses good night, the way that you held me",
    "I miss it every night. I wish I could tell you the things that I want",
    "but I can't. and It's haunts me when I dream at night",
    "Hayley, if you hear this, I'm sorry",
    "I know that I played games and acted retarded",
    "I know that my shame had my soul disregarded",
    "I know that you only wanted me, not the artist",
    "I'm sorry. I couldn't let go of the darkness.",
    "I was too scared if I did I'd be lost",
    "but Now I know God has my body regardless",
    "Loss after loss has my skull in despondence",
    "I wanted to love you. I know it was poisoned",
    "I know I was abusive when backed into a corner",
    "I know I was worthless, A thorn on your side at my best",
    "but I can't erase the way that my chest feels.",
    "Suppressed the river enough",
    "stress delivering drugs. I reject it. I guess my liver is plugged.",
    "head configured to duck",
    "memorized the cracks on the ground",
    "too much of a bitch, to lift my head up",
    "Lifetimes... pouring in the sea",
    "from the golden chalice",
    "soulless malice",
    "whispers underneath the willow tree (Oooooohhhhh)",
    "pillow talk is all I want",
    "I'm just a will o wisp",
    "Low key enough hide in a crowd",
    "The child inside",
    "you know who that is. (Oooooohhhhh)",
    "I love you so much,",
    "that I want your last name.",
    "I want to be yours.",
    "I'm sorry if that's not okay. (Oooooohhhhh)",
    "I can't remember a day",
    "that I felt content",
    "since the day that I left",
    "And flew away from the sound of your breath. (Oooooohhhhh)",
    "So many memories",
    "I can't just throw them away. (Oooooohhhhh)",
    "Come back, or don't",
    "either way, you'll remain. (Oooooohhhhh)",
    "Either way, you'll remain.",
    "I miss the scent of petrichor",
    "that we shared, while walking in the rain. (Oooooohhhhh)",
    "I love you.",
    "I'm sorry."
  ],
  annotations: [
  ...
  pacing: {
    ...
    verseSylPerBeat: 1.6,
    // rapped flow
    chorusSylPerBeat: 0.7,
    // belted, sustained delivery
    leadInS: 9,
    // glitch intro before the first sung line
    tailS: 6,
    // instrumental outro
...
const MAZE_SCREENSAVER = {
  ...
  // TODO: fill in actual title
  artist: "Vaelrix",
  model: "suno",
  modelVersion: "v4",
  duration: 180,
  // TODO: fill in actual duration (seconds)
  ...
  lyrics: [],
  // populated automatically by WhisperX transcript
  annotations: []
...
const PETRICHOR_WITH_ALIGNMENT = {
  ...PETRICHOR,
  wordTimings: petrichor_align_namespaceObject.wordTimings,
  alignmentMeta: (({ wordTimings: _, ...meta }) => meta)(petrichor_align_namespaceObject)
};
...
var renderContent = (Root) => {
  ...
  if (bundleMode.type === "index") {
    ...
    Promise.all(/* import() */[__webpack_require__.e(750), __webpack_require__.e(375), __webpack_require__.e(226)]).then(__webpack_require__.bind(__webpack_require__, 5375)).then(({ StudioInternals }) => {
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "scholomance-vite:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkscholomance_vite"] = self["webpackChunkscholomance_vite"] || [];
