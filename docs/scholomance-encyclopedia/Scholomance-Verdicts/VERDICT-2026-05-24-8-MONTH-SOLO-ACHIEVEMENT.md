...
| Verdict Class | SINGLE-AUDITOR (cross-jurisdictional; `gemini-*` and `codex` partials pending full reconciliation) |
| Status | RENDERED |
...
| **UX Friction** | **2 / 10** | ▼ | Core scroll editor + Truesight mode is clean; Fly.io deploy has an unresolved `0.0.0.0:8080` bind warning that currently prevents proxy reach |
...
### 7.1 — Immediate (This session / current deploy)

| Action | Owner | Severity | Cost | Reversibility | Success Criterion |
|---|---|---|---|---|---|
| Fix Fly.io `0.0.0.0:8080` bind — check `Dockerfile` `CMD` and `codex/server/index.js` HOST/PORT resolution | `gemini-*` / Angel | WARN | 1–2 agent-hours | cheap (redeploy) | `GET /health/live` returns 200 from external URL |
| Verify `https://scholomance-v12.fly.dev/health/live` returns `{status: "live"}` post-fix | Angel | WARN | 5 minutes | cheap | HTTP 200 confirmed from outside the machine |

### 7.2 — 30 Day

| Action | Owner | Severity | Cost | Reversibility | Success Criterion |
|---|---|---|---|---|---|
| Provision Cloudflare — create `wrangler.toml` for static asset CDN (Pages) or edge worker | Angel + `gemini-*` | WARN | 2–4 agent-hours | cheap | `wrangler deploy` exits 0; assets served from Cloudflare edge |
| Register `pathogen.rename-without-consumer-grep` in immunity pathogen registry | `codex` | WARN | 1 agent-hour | cheap | Pathogen appears in L2 adaptive scanner; `immune:scan` catches future rename regressions |
| Resolve stale `routes.js` doc reference flagged by COMB Tooth 5 | Angel | INFO | 15 minutes | cheap | COMB Tooth 5 reports 0 stale references |

### 7.3 — 90 Day

| Action | Owner | Severity | Cost | Reversibility | Success Criterion |
|---|---|---|---|---|---|
| File `gemini-*` and `codex` Partial Verdicts + Reconciliation for IMMUNE-SYSTEM and COGNITIVE-BUS | `gemini-*` + `codex` | INFO | 3–5 agent-hours per partial | cheap | Reconciliation documents exist; single-auditor verdicts marked SUPERSEDED |
| Add pre-commit hook: `grep -r '<renamed-symbol>'` guard for any export rename touching `codex/server/` or `codex/core/` | `gemini-*` | INFO | 2 agent-hours | cheap | Hook fires on next rename commit; no test failures from export renames in next quarter |
...
The conditions for re-render are: (1) the Fly.io bind warning is resolved and externally verified; (2) Cloudflare is provisioned; (3) any CRIT-tier violation surfaces in the next quarter's development arc. The Re-Render Due date of 2026-11-24 should be honoured regardless of condition triggers.
