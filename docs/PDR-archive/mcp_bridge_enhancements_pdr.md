...
**Priority:** High  
**Primary Goal:** Harden MCP stdio transport first, then extend the bridge with advanced coordination features
...
| Date | Change | Author |
|------|--------|--------|
| 2026-04-04 | Added Phase 0 transport reliability gate and clarified auth scope | Codex |
| 2026-04-02 | Clarified remote bearer-token access alongside local session login | Qwen Code |

**Rationale:** MCP work is blocked unless the stdio bridge can reliably survive process startup and complete `initialize`, `listResources`, and `listTools` across supported hosts. Remote bearer-token access remains valid, but local HTTP / CLI session login still exists and should not be described as removed.

---

## 1. Executive Summary

The MCP bridge (`codex/server/collab/mcp-bridge.js`) provides local stdio access to the collab control plane. This PDR now proposes a prerequisite transport-hardening phase followed by five enhancements to make it production-ready for advanced agent coordination:

0. **Transport reliability and handshake hardening** for stable MCP initialization
...
|------------|--------|---------|
| **Handshake is not sufficiently hardened** | MCP hosts can fail before tools/resources become available | Client hangs on `initialize` or receives connection closed |
...
1. **MCP hosts** need predictable `initialize`, `listResources`, and `listTools` behavior before any higher feature matters
2. **High-frequency traders** (agents) need real-time task state changes
3. **Pipeline orchestrators** need to stream large activity logs
4. **Batch processors** need to update multiple tasks atomically
5. **Collaborative agents** need direct communication channels

---

## 3. Product Goal

Transform the MCP bridge from a **basic CRUD interface** into a **reliable transport plus full-featured coordination protocol** that supports:

- Deterministic stdio handshake and protocol-clean startup
...
- Fixing host runtime bugs outside the repository; this PDR defines repo-side guarantees and diagnostics so external host failures can be isolated cleanly

---

## 5. Core Design Principles

### 5.1 Transport Reliability Is the Gate

No advanced MCP feature ships until the bridge satisfies a baseline reliability matrix:

- protocol-clean stdout during startup and request handling
- deterministic process lifetime under stdio
- successful `initialize`, `listResources`, and `listTools` against supported MCP hosts
- explicit diagnostics when a failure is outside repo control

### 5.2 Bytecode Is Priority

All streaming payloads, event notifications, and message payloads use bytecode encoding for determinism and compression.

### 5.3 Pure Analysis Never Touches Effects

MCP bridge remains a passthrough layer — no business logic, only transport enhancements.

### 5.4 Determinism Is Non-Negotiable

Streaming and subscriptions must produce identical output for identical input sequences.

### 5.5 Security Before Features
...
│  │  - Protocol-clean stdout/stderr discipline             │   │
│  │  - Process lifetime + shutdown handling                │   │
│  │  - Handshake probe + host compatibility diagnostics    │   │
│  └──────────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  Streaming Layer                                      │   │
│  │  - Chunked responses                                  │   │
...
### 8.0 Transport Reliability Layer

**Files:** `codex/server/collab/mcp-bridge.js` (existing), `scripts/debug-mcp-bridge.js` (new)

**Responsibilities:**
- Keep stdio startup protocol-clean
- Keep the bridge alive for the duration of an MCP session
- Provide a repeatable handshake probe outside any specific editor host
- Distinguish repo-local failures from host/runtime transport failures

**Deliverables:**
- stdout/stderr discipline for startup diagnostics
- explicit process lifecycle management for stdio mode
- a standalone probe script that verifies `initialize`, `listResources`, and `listTools`
- a host compatibility matrix recorded in docs
...
### Phase 0: Transport Reliability and Handshake Hardening
**Duration:** 1-2 days  
**Owner:** Codex  
**Deliverables:**
- protocol-clean startup for stdio MCP mode
- deterministic bridge process lifetime under stdio
- `scripts/debug-mcp-bridge.js` probe for `initialize`, `listResources`, and `listTools`
- documented host compatibility matrix for supported MCP clients

**QA Requirements:**
- `initialize` completes successfully in the standalone probe
- `listResources` and `listTools` complete successfully in the standalone probe
- no stdout contamination before or during handshake
- failures outside repo control are clearly attributable via diagnostics
...
|--------|-----------|-------------------|-----------|
| Transport Reliability | ✓ Startup lifecycle | ✓ Raw stdio handshake probe | ✓ Supported-host handshake matrix |
...
|-----------|--------|-------------|
| Initialize handshake | <1000ms | Spawn to initialize response |
...
- [ ] Phase 0 reliability gate passes before Phase 1 work begins
- [ ] `initialize`, `listResources`, and `listTools` succeed in the standalone probe
...
|------|--------|------------|
| **Host stdio incompatibility** | High — bridge appears broken when repo code is healthy | Maintain standalone probe and compatibility matrix; isolate host/runtime failures explicitly |
