 ...
 * Produces the same format as the /grimdesign skill, suitable for direct agent consumption.
 *
 * @param {string} intent
 * @param {import('../../core/grimdesign/signalExtractor').GrimSignal} signal
 * @param {import('../../core/grimdesign/decisionEngine').GrimDesignDecisions} decisions
...
export function registerCollabMcpBridge(server, service = collabService) {
    ...
    registerTool(server, 'grimdesign_analyze', {
        intent: z.string().min(1).max(500).describe(
            'Natural-language description of the UI component or surface to design. ' +
            'Examples: "cooldown indicator for a VOID-school agent", ' +
            '"corpus search result card for PSYCHIC school", ' +
            '"error state for a SONIC combat resolver".'
        ),
        component_name: z.string().max(64).optional().describe(
            'Optional PascalCase component name for the spec (e.g. "CooldownIndicator"). ' +
            'Defaults to "GrimComponent".'
        ),
...
function holdProcessOpenForStdio() {
    // In this environment, a stdio MCP child can exit immediately after connect()
    // unless the event loop has an additional referenced handle.
