...
import { measureTextWidth } from './adaptiveWhitespaceGrid';

const SPACING_FACTOR = 0.0; // Disabled to prevent misalignment with standard textarea
...
export function measureWithCorpusInference(
  ...
  for (const token of tokens) {
    ...
    // Use unified measureTextWidth from adaptiveWhitespaceGrid
    ...
    if (isWord && charCount > 0) {
      ...
      // Spacing adjustment disabled to maintain perfect alignment with substrate
