...
export function prefixProvider(context, engines) {
  ...
  if (ritualPredictionEngine && typeof ritualPredictionEngine.run === 'function') {
    ...
    if (Array.isArray(prediction?.candidates) && prediction.candidates.length > 0) {
      return prediction.candidates.slice(0, 20).map((candidate) => ({
        ...
        arbiter: {
          ...
          signals: {
            activation: candidate.activationScore,
            legality: candidate.legalityScore,
            semantic: candidate.semanticScore,
            phonetic: candidate.phoneticScore,
            connectedness: candidate.connectedness,
          },
        },
      }));
    }
