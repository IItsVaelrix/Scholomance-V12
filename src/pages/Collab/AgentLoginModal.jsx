...
const AgentLoginSchema = z.object({
    ...
    framework_origin: z.string().optional(),
...
const FRAMEWORK_OPTIONS = [
    { value: 'native', label: 'Native Mind', icon: '◈' },
    { value: 'langchain', label: 'LangChain', icon: '🦜' },
    { value: 'autogen', label: 'AutoGen', icon: '🤖' },
    { value: 'mcp-external', label: 'External MCP', icon: '🌐' },
];
        ...
        try {
            ...
            const registerResponse = await fetch('/collab/agents/register', {
                ...
                body: JSON.stringify({
                    ...
                    name: formData.name,
                    role: formData.role,
                    framework_origin: formData.framework_origin,
                    capabilities,
                }),
            });
                        ...
                        <div className="agent-login-modal__body">
                            {step === 'form' && (
                                <form onSubmit={handleSubmit} className="agent-login-form">
                                    ...
                                    <div className="agent-login-form__group">
                                        <span className="agent-login-form__label">Framework Origin</span>
                                        <div className="agent-login-form__frameworks" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
                                            {FRAMEWORK_OPTIONS.map((opt) => (
                                                <button
                                                    key={opt.value}
                                                    type="button"
                                                    className={`agent-login-form__framework-btn ${formData.framework_origin === opt.value ? 'selected' : ''}`}
                                                    onClick={() => updateField('framework_origin', opt.value)}
                                                    aria-pressed={formData.framework_origin === opt.value}
                                                    style={{
                                                        padding: '8px',
                                                        background: formData.framework_origin === opt.value ? 'var(--color-collab-gold-dim)' : 'rgba(0,0,0,0.2)',
                                                        border: `1px solid ${formData.framework_origin === opt.value ? 'var(--color-collab-gold)' : 'var(--border-collab-chrome)'}`,
                                                        borderRadius: '4px',
                                                        color: 'var(--color-collab-text)',
                                                        cursor: 'pointer',
                                                        display: 'flex',
                                                        alignItems: 'center',
                                                        gap: '8px',
