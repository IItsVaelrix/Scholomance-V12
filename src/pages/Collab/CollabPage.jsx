...
import { BugMarquee } from './components/Cabinet/BugMarquee.jsx';
import { BugDeckControls } from './components/Cabinet/BugDeckControls.jsx';
import { BugTerminalHeader } from './components/Terminal/BugTerminalHeader.jsx';
import { BugChecksumShrine } from './components/Wings/BugChecksumShrine.jsx';
import { BugRecoveryModule } from './components/Wings/BugRecoveryModule.jsx';
...
const AgentSchema = z.object({
    ...
    status: z.string(),
    role: z.string(),
    framework_origin: z.string().nullable().optional(),
...
const TaskSchema = z.object({
    ...
    file_paths: z.union([z.array(z.string()), z.string()]).nullable().optional().transform(v => {
        if (typeof v === 'string') {
...
const PipelineSchema = z.object({
    ...
    status: z.string(),
    current_stage: z.number().nullable().optional(),
    stages: z.union([z.array(z.any()), z.string()]).nullable().optional().transform(v => {
        if (typeof v === 'string') {
            try { return JSON.parse(v); } catch { return []; }
        }
        return v || [];
    }),
    results: z.union([z.object({}).passthrough(), z.string()]).nullable().optional().transform(v => {
        if (typeof v === 'string') {
            try { return JSON.parse(v); } catch { return {}; }
        }
        return v || {};
...
const ActivitySchema = z.object({
    ...
    action: z.string(),
    details: z.union([z.object({}).passthrough(), z.string()]).nullable().optional().transform(v => {
        if (typeof v === 'string') {
            try { return JSON.parse(v); } catch { return {}; }
        }
        return v || {};
...
const BugReportSchema = z.object({
    ...
    error_code_hex: z.string().nullable().optional(),
    bytecode: z.string().nullable().optional(),
    checksum_verified: z.union([z.boolean(), z.number()]).optional().transform(v => !!v),
    parseable: z.union([z.boolean(), z.number()]).optional().transform(v => !!v),
    auto_fixable: z.union([z.boolean(), z.number()]).optional().transform(v => !!v),
    source_type: z.string(),
    solution_bytecode: z.string().nullable().optional(),
    solution_ledger_status: z.string().nullable().optional(),
    corroborating_agents: z.union([z.array(z.string()), z.string()]).optional().transform(v => {
        if (typeof v === 'string') {
            try { return JSON.parse(v); } catch { return []; }
        }
        return v || [];
...
const LockSchema = z.object({
    ...
    mcp_active: z.union([z.boolean(), z.number()]).optional().transform(v => !!v),
    mcp_stream: z.union([z.object({}).passthrough(), z.string()]).nullable().optional().transform(v => {
        if (typeof v === 'string') {
...
export default function CollabPage() {
    ...
    const refresh = useCallback(async ({ silent = false } = {}) => {
        ...
        try {
            ...
            setLocks(parseCollabCollection(LockSchema, locksRaw, 'locks'));
            ...
            const bytecode = emitNetworkBytecodeError('/collab/sync', err.status || 0, { error: err.message });
            setError(bytecode);
            setStatus('error');
        }
    }, [checkMe]);
    ...
    // Handle bug actions
    const handleBugCreateTask = useCallback(async (bugId) => {
        setStatus('processing');
        try {
            const task = await fetchCollabJson(`/collab/bugs/${bugId}/create-task`, {
                method: 'POST',
            });
            refresh();
            // Optional: notify success
            console.log('Task created from bug:', task.id);
        } catch (err) {
            setError(err?.message || 'Failed to create task from bug');
            setStatus('error');
        }
    }, [refresh]);

    const handleBugStartPipeline = useCallback(async (bugId) => {
        setStatus('processing');
        try {
            // Step 1: Create task from bug
            const task = await fetchCollabJson(`/collab/bugs/${bugId}/create-task`, {
                method: 'POST',
            });
            // Step 2: Create pipeline for the task
            ...
            setError(err?.message || 'Failed to start pipeline for bug');
            setStatus('error');
        }
    }, [refresh]);
    ...
    const handleBugVerify = useCallback(async (bugId) => {
        try {
            ...
            setError(err?.message || 'Failed to verify bug');
            setStatus('error');
        }
    }, [handleBugUpdate]);
    ...
    const handlePrevBug = useCallback(() => {
        ...
        const nextIdx = currentBugIndex <= 0 ? bugs.length - 1 : currentBugIndex - 1;
        setSelectedBug(bugs[nextIdx]);
    }, [bugs, selectedBug]);

    const handleNextBug = useCallback(() => {
        if (bugs.length === 0) return;
        const currentBugIndex = selectedBug ? bugs.findIndex(b => b.id === selectedBug.id) : -1;
        const nextIdx = (currentBugIndex === -1 || currentBugIndex === bugs.length - 1) ? 0 : currentBugIndex + 1;
        setSelectedBug(bugs[nextIdx]);
    }, [bugs, selectedBug]);
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e) => {
            ...
            // Navigation works even if no bug is selected
            if (e.key === 'ArrowUp') {
                e.preventDefault();
                handlePrevBug();
                return;
            ...
            switch (e.key.toLowerCase()) {
                case 't': handleBugCreateTask(selectedBug.id); break;
                case 'p': handleBugStartPipeline(selectedBug.id); break;
                case 'a': handleBugAssign(selectedBug.id); break;
                case 'd': handleBugDuplicate(selectedBug.id); break;
                case 'v': handleBugVerify(selectedBug.id); break;
                case 'c': 
                    if (selectedBug.bytecode) {
                        navigator.clipboard.writeText(selectedBug.bytecode);
                    }
                    break;
                case 'enter':
                    setIsBugDrawerOpen(true);
    ...
    }, [activeTab, selectedBug, handlePrevBug, handleNextBug, handleBugCreateTask, handleBugStartPipeline, handleBugAssign, handleBugDuplicate, handleBugVerify]);
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'bugs': {
                return (
                    <BugPageCabinetShell
                        marquee={<BugMarquee severity={selectedBug?.severity} />}
                        deck={
                            <BugDeckControls 
                                onPrev={handlePrevBug} 
                                onNext={handleNextBug} 
                                onSelect={() => selectedBug && setIsBugDrawerOpen(true)}
                                onCreateTask={() => selectedBug && handleBugCreateTask(selectedBug.id)}
                                onStartPipeline={() => selectedBug && handleBugStartPipeline(selectedBug.id)}
                                onAssign={() => selectedBug && handleBugAssign(selectedBug.id)}
                                onVerify={() => selectedBug && handleBugVerify(selectedBug.id)}
                                onDuplicate={() => selectedBug && handleBugDuplicate(selectedBug.id)}
                            />
                        }
                        leftWing={
                            <BugQueueRail 
                            ...
                            <div className="wing-content">
                                ...
                                <BugRecoveryModule 
                                    hints={selectedBug?.recovery_hints || (selectedBug ? [
                                        `Investigate ${selectedBug.module_id || 'unidentified module'} for state drift`,
                                        `Verify ${selectedBug.category} invariant in runtime logic`,
                                        `Check bytecode checksum against local source`
                                    ] : [])} 
                                />
                                <div className="wing-rail">
                    ...
                    >
                        <div className="bug-cabinet-terminal">
                            {selectedBug ? (
                                <div className="terminal-inner">
                                    <BugTerminalHeader bug={selectedBug} />
                                    <div className="terminal-scroll-area">
                                        <div className="terminal-content">
                                            <h4 style={{ color: '#aaa', fontSize: '10px', marginBottom: '10px', fontFamily: "'Press Start 2P', cursive" }}>SUMMARY</h4>
                                            <p style={{ color: '#fff', fontSize: '14px', lineHeight: '1.6', marginBottom: '20px' }}>{selectedBug.summary}</p>
                                            
                                            <h4 style={{ color: '#aaa', fontSize: '10px', marginBottom: '10px', fontFamily: "'Press Start 2P', cursive" }}>BYTECODE PAYLOAD</h4>
                                            <pre style={{ 
                                                color: '#00ff00', 
                                                fontSize: '11px', 
                                                ...
                                                {selectedBug.bytecode || 'NO BYTECODE EMITTED'}
                                            </pre>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="terminal-empty">
                                    <DiagnosticGhost />
                                    <h3 style={{ color: '#fff', marginTop: '20px' }}>NO TARGET LOADED</h3>
                                    <p style={{ color: '#666' }}>Select a bug from the queue rail</p>
                                </div>
                            )}
                        </div>
                    </BugPageCabinetShell>
