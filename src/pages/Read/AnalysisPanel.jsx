...
function PatternChip({ letter, vowelFamily = null, vowelColors = {}, vowelColorResolver = null }) {
  const color = (vowelFamily && vowelColors[vowelFamily])
    ? (typeof vowelColors[vowelFamily] === 'string' ? vowelColors[vowelFamily] : vowelColors[vowelFamily].color)
    : (vowelColorResolver && vowelFamily ? vowelColorResolver(vowelFamily, 0) : patternColor(letter));
...
PatternChip.propTypes = { 
  letter: PropTypes.string.isRequired,
  vowelFamily: PropTypes.string,
  vowelColors: PropTypes.object,
  vowelColorResolver: PropTypes.func,
};
                ...
                <div className="analyze-pattern-row">
                  {patternLetters.map((letter, i) => {
                    ...
                    const firstWordInGroup = (lineIdx !== undefined && rhymeAstrology?.inspector?.anchors)
                      ...
                      : null;
                    const vFamily = firstWordInGroup?.primaryStressedVowelFamily || firstWordInGroup?.terminalVowelFamily || null;
                    ...
                    return (
                      <PatternChip 
                        ...
                        vowelColorResolver={vowelColorResolver}
                      />
                    );
                  })}
                </div>
            ...
            {groupsExpanded && (
              ...
              >
                {groupEntries.map(([label, lineIndices]) => {
                  const firstWordInGroup = rhymeAstrology?.inspector?.anchors?.find(
                    a => lineIndices.includes(a.lineIndex) && a.isLineEnd
                  );
                  const vFamily = firstWordInGroup?.primaryStressedVowelFamily || firstWordInGroup?.terminalVowelFamily || null;
                  
                  const color = (vFamily && vowelColors[vFamily])
                    ? (typeof vowelColors[vFamily] === 'string' ? vowelColors[vFamily] : vowelColors[vFamily].color)
                    : (vowelColorResolver && vFamily ? vowelColorResolver(vFamily, 0) : patternColor(label));
