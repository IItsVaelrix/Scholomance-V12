...
function getRatingFromScore(score) {
  const numeric = Number(score) || 0;
  if (numeric >= 90) return 'Godlike';
  if (numeric >= 75) return 'Master';
  if (numeric >= 60) return 'Adept';
  return 'Neophyte';
}
...
function HeuristicBar({ trace, index, signal }) {
  ...
  // Derive per-heuristic signal if available for stat overlays
  const heuristicSignal = signal ? {
    ...signal,
    statScores: [{ 
      stat: trace.heuristic.toUpperCase(), 
      rating: getRatingFromScore(trace.rawScore * 100) 
    }]
  } : null;

  return (
    <AnimatedSurface signal={heuristicSignal} className="heuristic-item">
...
export default function HeuristicScorePanel({ scoreData, genreProfile, visible, isEmbedded = false, onClose = null, pixelBrainSignal = null }) {
  ...
  const content = (
    <AnimatedSurface 
      signal={pixelBrainSignal} 
      className={`score-panel-surface ${pixelBrainSignal?.burstUnlockStat ? 'burst-active score-panel' : ''}`}
    >
      {!isEmbedded && (
      ...
      <div className="heuristic-breakdown">
        {scoreData.traces.map((trace, i) => (
          <HeuristicBar key={trace.heuristic} trace={trace} index={i} signal={pixelBrainSignal} />
        ))}
      </div>
