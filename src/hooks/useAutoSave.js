...
export function useAutoSave(inputs, options = {}) {
  ...
  const bumpAutosaveContext = useCallback((initialId, initialTitle, initialContent) => {
    autosaveContextRef.current += 1;
    queuedAutosaveRef.current = null;
    autosaveScrollIdRef.current = initialId || null;
    
    const idStr = initialId || "new";
    const titleStr = String(initialTitle || "").trim();
    const contentStr = String(initialContent || "");
    lastAutosaveFingerprintRef.current = `${idStr}|${titleStr}|${contentStr}`;
