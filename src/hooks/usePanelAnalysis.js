...
import { computeDominantSchool, computeSchoolWeights } from "../data/schools.js";
...
const ANALYSIS_DEBOUNCE_MS = 650; // Balance editor responsiveness against server-side analysis cost.
...
function getPanelAnalysisEndpoint() {
  return buildAuthorityUrl("/api/analysis/panels");
}

function buildSchoolSignalPayload(wordAnalyses) {
  const vowelFamilyDistribution = {};

  (Array.isArray(wordAnalyses) ? wordAnalyses : []).forEach((wordAnalysis) => {
    const familyId = normalizeVowelFamily(wordAnalysis?.vowelFamily);
    if (!familyId) return;
    vowelFamilyDistribution[familyId] = (vowelFamilyDistribution[familyId] || 0) + 1;
  });

  const schoolWeights = computeSchoolWeights(vowelFamilyDistribution);
  return {
    schoolWeights,
    dominantSchool: computeDominantSchool(schoolWeights),
  };
}
...
function normalizePanelPayload(rawPayload) {
  ...
  const analysis = payload.analysis
    ? {
      ...
      schoolWeights: payload.analysis.schoolWeights && typeof payload.analysis.schoolWeights === "object"
        ? payload.analysis.schoolWeights
        : {},
      dominantSchool: typeof payload.analysis.dominantSchool === "string"
        ? payload.analysis.dominantSchool
        : null,
...
async function runClientSideAnalysis(text) {
    ...
    for (const line of analysis.lines) {
      ...
      for (const w of line.words) {
        ...
        wordAnalyses.push({
          ...
          primaryStressedVowelFamily: w.primaryStressedVowelFamily || a?.primaryStressedVowelFamily || null,
          terminalVowelFamily: w.terminalVowelFamily || a?.terminalVowelFamily || null,
          syllableCount: w.syllableCount || a?.syllableCount || 0,
          rhymeKey: w.rhymeKey || a?.rhymeKey || null,
          rhymeTailSignature: w.rhymeTailSignature || a?.rhymeTailSignature || w.rhymeKey || a?.rhymeKey || null,
  ...
  const schoolSignals = buildSchoolSignalPayload(wordAnalyses);
  ...
  return {
    data: {
      analysis: {
        ...
        schoolWeights: schoolSignals.schoolWeights,
        dominantSchool: schoolSignals.dominantSchool,
        syntaxSummary,
