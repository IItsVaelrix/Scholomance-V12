/**
 * Phaser WebGL PostFXPipeline for "scimitar blade energize" (item.scimitar.hd.v2-lit.blade.energize)
 * Generated deterministically from Scholomance custom shader packet.
 */
export class ItemScimitarHdV2LitBladeEnergizePipeline extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline {
  constructor(game) {
    super({
      game,
      name: 'item.scimitar.hd.v2-lit.blade.energize',
...
//   - BOLT_TINT (#FFFFFF)
//   - ENGRAVING_DENSITY (0.0285)
//   - host part: blade.curved over v in [0.0000, 0.6146]
//
// All motion is deterministic — sin(u_time * k) and pbHash(vec2) only.
const vec3 BOLT_TINT = vec3(1.0000, 1.0000, 1.0000);
const float ENGRAVING_DENSITY = 0.0285;
