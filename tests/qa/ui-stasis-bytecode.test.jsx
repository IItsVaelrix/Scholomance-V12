...
describe('UI Stasis — Clickable Elements', () => {
  describe('Button Click Handlers', () => {
    ...
    it('should recover from click handler errors', async () => {
      ...
      // We verify that a component remains interactive even after a handler failure.
      ...
      const RecoveryComponent = () => {
        ...
        const handleClick = () => {
          if (shouldFail) {
            setShouldFail(false); // Reset for recovery
        ...
        return (
          <div>
            ...
            <button onClick={handleClick} data-testid="error-btn">
              Click Me
            </button>
            <button onClick={() => setShouldFail(true)} data-testid="fail-trigger">
              Fail Next
            </button>
          </div>
        );
      ...
      const { container } = render(<RecoveryComponent />);
      const button = screen.getByTestId('error-btn');
      const failTrigger = screen.getByTestId('fail-trigger');
      const countLabel = screen.getByTestId('count');
      ...
      fireEvent.click(failTrigger);
      
      // Suppress console.error and window error for the failing click
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      
      // Prevent the error from reaching Vitest's unhandled error detector
      ...
      // We use try/catch to catch the error thrown by JSDOM during dispatch
      try {
        fireEvent.click(button);
      } catch (e) {
