...
Options:
  ...
  --timeout-ms <ms>   Per-step timeout in milliseconds (default: 10000)
  --transport <mode>  Probe transport: auto, sdk, raw, raw-interactive, or memory (default: auto)
  --no-read-status    Skip the collab://status read after listResources/listTools
  --probe-tool        Also call mcp_scholomance_collab_status_get after listing tools
...
async function main() {
    ...
    if (!['auto', 'sdk', 'raw', 'raw-interactive', 'memory'].includes(transport)) {
    ...
    const report = await runCollabMcpProbe({
        ...
        probeToolExecution: Boolean(options['probe-tool']),
        transportMode: transport === 'auto' ? 'sdk' : transport,
        allowRawFallback: transport === 'auto',
    });
